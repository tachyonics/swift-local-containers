...
- [ ] **Investigate: replace `docker build` shell-out with a pure-Swift BuildKit gRPC client.** `DockerContainerRuntime.buildImage` currently shells out to the local `docker` CLI for image builds (see `DockerBuild.swift`). This was chosen over the Engine API REST `/build` endpoint because modern Dockerfiles depend on BuildKit features (`# syntax=docker/dockerfile:1`, `RUN --mount=type=cache`, secrets, etc.) and BuildKit-over-REST requires a hijacked-HTTP gRPC session protocol that's substantial to implement (thousands of LOC, swift-protobuf + grpc-swift surface, ongoing maintenance against BuildKit protocol changes). Shell-out works against a local daemon and matches what most other test-helper tools do. **Worth revisiting only if** real user demand surfaces for: (a) remote daemon support without docker CLI on the test machine, (b) sandboxed environments where `Foundation.Process` can't spawn, or (c) programmatic streaming-progress integration with swift-log that goes beyond what we can do by parsing `docker build` output. Until then, the shell-out is the pragmatic answer; this TODO captures the option so future-us doesn't have to re-derive it.
